...
class DbTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_funda_autoreply_settings_contact_and_attempts_are_persisted(self):
        await db.init_db()
        await db.save_filters(
            chat_id=10,
            max_price=1800,
        ...
        filters = await db.get_filters(10)
        self.assertFalse(filters["funda_autoreply_enabled"])
        self.assertFalse(filters["funda_autoreply_manual_approval"])
        self.assertEqual(filters["funda_autoreply_template"], "")
        self.assertEqual(filters["funda_autoreply_email"], "")

        await db.set_funda_autoreply_enabled(10, True)
        await db.set_funda_autoreply_manual_approval(10, True)
        await db.set_funda_autoreply_template(10, "Hello about {title}")
        await db.set_funda_autoreply_contact(
            10,
            "person@example.test",
            "First",
            "Last",
        ...
        filters = await db.get_filters(10)
        self.assertTrue(filters["funda_autoreply_enabled"])
        self.assertTrue(filters["funda_autoreply_manual_approval"])
        self.assertEqual(filters["funda_autoreply_template"], "Hello about {title}")
        self.assertEqual(filters["funda_autoreply_email"], "person@example.test")
        self.assertEqual(filters["funda_autoreply_first_name"], "First")
        self.assertEqual(filters["funda_autoreply_last_name"], "Last")
        self.assertEqual(filters["funda_autoreply_phone"], "+31612345678")

        self.assertTrue(
            await db.reserve_funda_auto_reply(
                10,
                "funda-1",
        ...
        self.assertEqual(await db.get_funda_autoreply_stats(10), {"pending": 1})
        await db.update_funda_auto_reply(10, "funda-1", "sent", "")
        self.assertEqual(await db.get_funda_autoreply_stats(10), {"sent": 1})

    async def test_funda_autoreply_failures_can_be_retried_after_cooldown(self):
        await db.init_db()
        ...
        await db.update_funda_auto_reply(10, "funda-retry", "submit_unknown", "No confirmation")
        with (
            patch.object(db.config, "FUNDA_AUTOREPLY_RETRY_COOLDOWN_SECONDS", 0),
            patch.object(db.config, "FUNDA_AUTOREPLY_MAX_ATTEMPTS_PER_LISTING", 2),
        ):
            retry_rows = await db.get_retryable_funda_auto_replies(10, limit=5)

            self.assertEqual(len(retry_rows), 1)
            self.assertEqual(retry_rows[0]["listing_id"], "funda-retry")
            self.assertEqual(retry_rows[0]["price"], "EUR 1500")
            self.assertTrue(
                await db.reserve_funda_auto_reply(
                    10,
                    "funda-retry",
            ...
            await db.update_funda_auto_reply(10, "funda-retry", "submit_unknown", "No confirmation")
            self.assertEqual(await db.get_retryable_funda_auto_replies(10, limit=5), [])
